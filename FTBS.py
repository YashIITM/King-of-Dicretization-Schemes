...
def FTBS(u,CFL,deltaX,deltaT = 0.01):
    nx = len(u[0])
    nt = len(u)-1
    c = CFL*deltaX/deltaT
    for n in range(nt):
        ...
        u[n+1,0] = u[n,0] + (c*deltaT/deltaX)*(u[n,nx-2]-u[n,0])
    return u
...
line2, = ax.plot([], [], lw=2,label='Numerical Solution using FTBS')
def init():
   line.set_data([], [])
   line2.set_data([],[])
   return line,line2,
...
plt.title("Numerical Solution using FTBS ")
plt.show()
